...
use Userstory\I18n\Helper\UserGroupHelper;
...
trait IblockTrait
{
     ...
     * @return array
     */
    public function getIblockIfExists($code, $typeId = ''): array
    {
        ...
        $this->throwException(
            __METHOD__,
            Loc::getMessage(
                Helper::MESSAGE_PREFIX . 'ERR_IB_NOT_FOUND',
    ...
    public function getIblockSites($iblockId)
    {
        ...
        return (new IblockHelper())->fetchAll($dbres, false, 'LID');
    }
    ...
    public function exportIblock($iblockId)
    {
        ...
        $this->throwException(
            __METHOD__,
            Loc::getMessage(
                Helper::MESSAGE_PREFIX . 'ERR_IB_CODE_NOT_FOUND',
                ['#IBLOCK_ID#' => $iblockId]
            )
        );
    }
    ...
    public function getIblockUid($iblock)
    {
        ...
        if (empty($iblock['IBLOCK_TYPE_ID'])) {
            $this->throwException(
                __METHOD__,
                Loc::getMessage(
                    Helper::MESSAGE_PREFIX . 'ERR_TYPE_OF_IB_NOT_FOUND',
                    ['#IBLOCK_ID#' => $iblock['ID']]
                )
            );
        }
